		...
		if (world.provider.getDimensionType() == DimensionType.OVERWORLD) {
			...
			if (world.getBlockState(pos).isTopSolid()) {
				...
				for (int count = 0; count < total; ++count) {
					EntityHuman human = random.nextBoolean() ? new EntityFemale(world) : new EntityMale(world);
					human.setPosition(pos.getX() + (random.nextInt(total) - 6), world.getTopSolidOrLiquidBlock(pos).getY(), pos.getZ() + (random.nextInt(total) - 6));
